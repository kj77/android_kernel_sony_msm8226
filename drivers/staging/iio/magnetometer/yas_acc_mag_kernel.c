...
static int yas_pseudo_irq_enable(struct iio_dev *indio_dev)
{
	...
	if (!atomic_cmpxchg(&st->pseudo_irq_enable, 0, 1)) {
		mutex_lock(&st->lock);
		st->am.set_enable(YAS_TYPE_ACC|YAS_TYPE_MAG, 1);
...
static int yas_pseudo_irq_disable(struct iio_dev *indio_dev)
{
	...
	if (atomic_cmpxchg(&st->pseudo_irq_enable, 1, 0)) {
		...
		mutex_lock(&st->lock);
		st->am.set_enable(YAS_TYPE_ACC|YAS_TYPE_MAG, 0);
		mutex_unlock(&st->lock);
	}
	return 0;
}
...
static irqreturn_t yas_trigger_handler(int irq, void *p)
{
	...
	data = (int32_t *) kmalloc(datasize, GFP_KERNEL);
	if (data == NULL) {
	...
	if (!bitmap_empty(indio_dev->active_scan_mask, indio_dev->masklength)) {
		...
		for (i = 0; i < 3; i++) {
			if (test_bit(i, indio_dev->active_scan_mask)) {
				data[j] = st->accel_data[i];
				j++;
			}
		}
		for (i = 0; i < 3; i++) {
			if (test_bit(i+3, indio_dev->active_scan_mask)) {
				data[j] = st->compass_data[i];
				j++;
			}
		}
		len = j * 4;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	sscanf(buf, "%d\n", &position);
	mutex_lock(&st->lock);
	ret = st->am.set_position(position);
	mutex_unlock(&st->lock);
	if (ret < 0)
		return -EFAULT;
	return count;
		...
		const char *buf, size_t count)
{
	...
	st->am.set_delay(delay);
	mutex_unlock(&st->lock);
	return count;
}

#if YAS_MAG_DRIVER == YAS_MAG_DRIVER_YAS535
static ssize_t yas_acc_average_sample_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct yas_state *st = iio_priv(indio_dev);
	int8_t acc_average_sample;
	int ret;
	mutex_lock(&st->lock);
	ret = st->am.ext(YAS535_ACC_GET_AVERAGE_SAMPLE, &acc_average_sample);
	mutex_unlock(&st->lock);
	if (ret < 0)
		return -EFAULT;
	return sprintf(buf, "%d\n", acc_average_sample);
...
static ssize_t yas_acc_average_sample_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct yas_state *st = iio_priv(indio_dev);
	...
	int ret;
	sscanf(buf, "%d\n", &tmp);
	acc_average_sample = (int8_t)tmp;
	mutex_lock(&st->lock);
	ret = st->am.ext(YAS535_ACC_SET_AVERAGE_SAMPLE, &acc_average_sample);
	mutex_unlock(&st->lock);
	if (ret < 0)
		return -EFAULT;
	return count;
		...
		struct device_attribute *attr, char *buf)
{
	...
	int ret;
	mutex_lock(&st->lock);
	ret = st->am.ext(YAS535_MAG_GET_AVERAGE_SAMPLE, &mag_average_sample);
	mutex_unlock(&st->lock);
	if (ret < 0)
		return -EFAULT;
	return sprintf(buf, "%d\n", mag_average_sample);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&st->lock);
	ret = st->am.ext(YAS535_MAG_SET_AVERAGE_SAMPLE, &mag_average_sample);
	mutex_unlock(&st->lock);
	if (ret < 0)
		return -EFAULT;
	return count;
		...
		struct device_attribute *attr, char *buf)
{
	...
	int ret;
	mutex_lock(&st->lock);
	ret = st->am.ext(YAS535_MAG_GET_HW_OFFSET, hard_offset);
	mutex_unlock(&st->lock);
	if (ret < 0)
		return -EFAULT;
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	mutex_lock(&st->lock);
	ret = st->am.ext(YAS535_MAG_SET_HW_OFFSET, hard_offset);
	mutex_unlock(&st->lock);
	if (ret < 0)
		return -EFAULT;
	return count;
...
static ssize_t yas_acc_self_test_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct yas_state *st = iio_priv(indio_dev);
	struct yas535_acc_self_test_result r;
	int ret;
	mutex_lock(&st->lock);
	ret = st->am.ext(YAS535_ACC_SELF_TEST, &r);
	mutex_unlock(&st->lock);
		...
		struct device_attribute *attr, char *buf)
{
	struct iio_dev *indio_dev = dev_get_drvdata(dev);
	struct yas_state *st = iio_priv(indio_dev);
	struct yas535_mag_self_test_result r;
	int ret;
	mutex_lock(&st->lock);
	ret = st->am.ext(YAS535_MAG_SELF_TEST, &r);
	mutex_unlock(&st->lock);
		...
		struct device_attribute *attr, char *buf)
{
	...
	int ret;
	mutex_lock(&st->lock);
	ret = st->am.ext(YAS535_MAG_SELF_TEST_NOISE, xyz_raw);
	mutex_unlock(&st->lock);
	if (ret < 0)
		return -EFAULT;
...
static void yas_work_func(struct work_struct *work)
{
	...
	mutex_lock(&st->lock);
	acc = st->am.get_enable(YAS_TYPE_ACC);
	mag = st->am.get_enable(YAS_TYPE_MAG);
	if (acc && mag) {
		ret = st->am.measure(YAS_TYPE_ACC|YAS_TYPE_MAG, data, 2);
		if (ret == 2) {
			for (i = 0; i < 2; i++) {
				...
				if (data[i].type == YAS_TYPE_MAG)
					set_vector(st->compass_data,
							data[i].xyz.v);
			}
		}
	} else if (acc) {
		ret = st->am.measure(YAS_TYPE_ACC, data, 1);
		if (ret == 1) {
			for (i = 0; i < 3; i++)
				st->accel_data[i] = data[0].xyz.v[i]
					- st->accel_bias[i];
		}
	} else {
		ret = st->am.measure(YAS_TYPE_MAG, data, 1);
		if (ret == 1) {
			for (i = 0; i < 3; i++)
				st->compass_data[i] = data[0].xyz.v[i];
		}
	}
	mutex_unlock(&st->lock);
	if (0 < ret)
...
#if YAS_MAG_DRIVER == YAS_MAG_DRIVER_YAS535
static IIO_DEVICE_ATTR(acc_average_sample, S_IRUSR|S_IWUSR,
		yas_acc_average_sample_show, yas_acc_average_sample_store, 0);
...
static IIO_DEVICE_ATTR(acc_self_test, S_IRUSR, yas_acc_self_test_show, NULL, 0);
static IIO_DEVICE_ATTR(mag_self_test, S_IRUSR, yas_mag_self_test_show, NULL, 0);
static IIO_DEVICE_ATTR(mag_self_test_noise, S_IRUSR, yas_mag_self_test_noise_show,
		NULL, 0);
#endif
...
#if YAS_MAG_DRIVER == YAS_MAG_DRIVER_YAS535
	&iio_dev_attr_acc_average_sample.dev_attr.attr,
	&iio_dev_attr_mag_average_sample.dev_attr.attr,
	&iio_dev_attr_hard_offset.dev_attr.attr,
	&iio_dev_attr_acc_self_test.dev_attr.attr,
	&iio_dev_attr_mag_self_test.dev_attr.attr,
	&iio_dev_attr_mag_self_test_noise.dev_attr.attr,
#endif
	NULL
};
...
static int yas_probe(struct i2c_client *i2c, const struct i2c_device_id *id)
{
	...
	indio_dev->name = YAS_ACC_MAG_NAME;
	...
	st->am.callback.device_open = yas_device_open;
	st->am.callback.device_close = yas_device_close;
	st->am.callback.device_read = yas_device_read;
	st->am.callback.device_write = yas_device_write;
	st->am.callback.usleep = yas_usleep;
	st->am.callback.current_time = yas_current_time;
	...
	ret = yas_acc_mag_driver_init(&st->am);
	if (ret < 0) {
		ret = -EFAULT;
...
MODULE_DESCRIPTION("Yamaha Acceleration and Magnetometer I2C driver");

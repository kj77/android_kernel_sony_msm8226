...
#ifdef CONFIG_COMPAT
	.compat_ioctl = ecs_ctrl_compat_ioctl,
#endif
};
...
//    printk("+++ ecs_ctrl_ioctl +++\n");
//    printk("+++ ECOMPASS_IOC_SET_MFLAG %d +++\n", ECOMPASS_IOC_SET_MFLAG);
//    printk("+++ ECOMPASS_IOC_SET_OFLAG %d+++\n", ECOMPASS_IOC_SET_OFLAG);
//    printk("+++ ECOMPASS_IOC_SET_GRVFLAG %d+++\n", ECOMPASS_IOC_SET_GRVFLAG);

    printk("ecs_ctrl_ioctl ECOMPASS_IOC_SET_MFLAG cmd %d %u \n", ECOMPASS_IOC_SET_MFLAG, cmd);
	...
	case ECOMPASS_IOC_SET_MFLAG:
        ...
        printk("33333333333333333333333333\n");
		break;
	...
	default:
		break;
            ...
            printk("ecs_ctrl_ioctl return 0\n");

	return 0;
}

#ifdef CONFIG_COMPAT
static long ecs_ctrl_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	long ret = 0;

shoud build fail here !!!

    printk("+++ ecs_ctrl_compat_ioctl +++\n");
	...
	switch (cmd) {
	case COMPAT_ECOMPASS_IOC_SET_MODE:
		break;
	case COMPAT_ECOMPASS_IOC_SET_DELAY:
   		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_SET_DELAY, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_DELAY is failed!\n");
		}
		break;
	...
	case COMPAT_ECOMPASS_IOC_SET_AFLAG:
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_SET_AFLAG, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_AFLAG is failed!\n");
		}
		break;
	case COMPAT_ECOMPASS_IOC_GET_AFLAG:
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_GET_AFLAG, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_GET_AFLAG is failed!\n");
		}
		break;
	case COMPAT_ECOMPASS_IOC_SET_MFLAG:
            printk("+++ COMPAT_ECOMPASS_IOC_SET_MFLAG +++\n", COMPAT_ECOMPASS_IOC_SET_MFLAG);
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_SET_MFLAG, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_MFLAG is failed!\n");
		}
		break;
	case COMPAT_ECOMPASS_IOC_GET_MFLAG:
		...
		break;
	case COMPAT_ECOMPASS_IOC_SET_OFLAG:
            printk("+++ COMPAT_ECOMPASS_IOC_SET_OFLAG %d +++\n", COMPAT_ECOMPASS_IOC_SET_OFLAG);
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_SET_OFLAG, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_OFLAG is failed!\n");
		}
		break;
	case COMPAT_ECOMPASS_IOC_GET_OFLAG:
		...
		break;
  #ifdef KitKat
	case COMPAT_ECOMPASS_IOC_SET_GRVFLAG:
            printk("+++ COMPAT_ECOMPASS_IOC_SET_GRVFLAG %d +++\n", COMPAT_ECOMPASS_IOC_SET_GRVFLAG);
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_SET_GRVFLAG, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_GRVFLAG is failed!\n");
		}
		break;
	case COMPAT_ECOMPASS_IOC_GET_GRVFLAG:
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_GET_GRVFLAG, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_GET_GRVFLAG is failed!\n");
		}
		break;
  #endif

	case COMPAT_ECOMPASS_IOC_SET_APARMS:
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_SET_APARMS, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_APARMS is failed!\n");
		}
		break;
	case COMPAT_ECOMPASS_IOC_GET_APARMS:
		break;
	case COMPAT_ECOMPASS_IOC_SET_MPARMS:
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_SET_MPARMS, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_MPARMS is failed!\n");
		}
		break;
	case COMPAT_ECOMPASS_IOC_GET_MPARMS:
		break;
	case COMPAT_ECOMPASS_IOC_SET_OPARMS_YAW:
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_SET_OPARMS_YAW, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_OPARMS_YAW is failed!\n");
		}
		break;
	case COMPAT_ECOMPASS_IOC_GET_OPARMS_YAW:
		break;
	case COMPAT_ECOMPASS_IOC_SET_OPARMS_PITCH:
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_SET_OPARMS_PITCH, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_OPARMS_PITCH is failed!\n");
		}
		break;
	case COMPAT_ECOMPASS_IOC_GET_OPARMS_PITCH:
		break;
	case COMPAT_ECOMPASS_IOC_SET_OPARMS_ROLL:
		ret = file->f_op->unlocked_ioctl(file, ECOMPASS_IOC_SET_OPARMS_ROLL, (unsigned long)arg64);
		if(ret < 0)
		{
			printk(KERN_ERR "COMPAT_ECOMPASS_IOC_SET_OPARMS_ROLL is failed!\n");
		}
		break;
	case COMPAT_ECOMPASS_IOC_GET_OPARMS_ROLL:
		break;
